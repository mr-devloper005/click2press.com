...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  const taskLabel = SITE_CONFIG.tasks.find((item) => item.key === task)?.label || 'Media'
  const strong = index % 5 === 0

  return (
    <Link href={href} className={`group block overflow-hidden bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-[0_24px_70px_rgba(6,70,83,.12)] ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-black ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <span className="absolute right-4 top-4 bg-[var(--slot4-accent)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        ...
        <h2 className="mt-4 line-clamp-3 text-2xl font-black leading-tight text-[var(--slot4-page-text)]">{post.title}</h2>
        {summary ? <p className="mt-4 line-clamp-3 text-sm leading-7 text-[var(--slot4-muted-text)]">{summary}</p> : null}
        <span className="mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-[var(--slot4-dark-bg)] group-hover:text-[var(--slot4-accent)]">Open result <ArrowRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-panel-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[1260px] px-4 py-12 sm:px-6 lg:px-8">
          <div className="grid overflow-hidden bg-white shadow-[0_24px_70px_rgba(6,70,83,.10)] md:grid-cols-[0.85fr_1.15fr]">
            ...
            <form action="/search" className="self-center p-6 sm:p-10 lg:p-14">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center bg-[var(--slot4-dark-bg)] px-6 text-xs font-black uppercase tracking-[0.18em] text-white transition hover:bg-[var(--slot4-accent)]" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 py-10">
            ...
            <Link href="/updates" className="inline-flex items-center gap-2 bg-white px-5 py-3 text-xs font-black uppercase">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
